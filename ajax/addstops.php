$(function( ) {
	...
	var userIdIndex = location.href.indexOf('?userid=') + 8;
	var userId = "";
	if (userIdIndex >= 8) userId = location.href.substr(userIdIndex);
	
	if (userId.length != 0) {
		$('.form-group').hide();
		$('#stopmap-div').show();
		$('button').html('Add Stops');
	}
	...
	function initMap() {
		...
		var refreshMap = debounce(function() {
			...
			var url = 'http://localhost/barracks/ajax/get-adoptable-stops.php' +
	...
	$('#signup-form button').click(function(e) {
		...
		var data = {userid: userId, name: name, email: email, phone: phone, comment: comment}
	...
	function getStopDescription(marker) {
		...
		if (marker.selected == false) {
			if (marker.reason == undefined) {
				return "<b>" + marker.stopname + "</b><br/>" + marker.stopid + "<br/>Available<br/><a href='#' class='adopt-stop'>Adopt this stop</a></div></div>";
			}
			else {
//				return "<b>" + marker.stopname + "</b><br/>" + marker.stopid + "<br/>WARNING: " + displayedReason + "<br/><a href='#' class='adopt-stop'>Adopt this stop anyway</a></div></div>";				
				return "<b>" + marker.stopname + "</b><br/>" + marker.stopid + "<br/>WARNING: " + displayedReason + "<br/><a href='mailto:contact@martaarmy.org' >Contact us if you wish to adopt this stop</a></div></div>";				
			}
		}
		else {
			if (marker.reason == undefined) {
				return "<b>" + marker.stopname + "</b><br/>" + marker.stopid + "<br/>Available<br/><a href='#' class='adopt-stop'>Unselect this stop</a></div></div>";
			}
			else {
				return "<b>" + marker.stopname + "</b><br/>" + marker.stopid + "<br/>WARNING: " + displayedReason + "<br/><a href='#' class='adopt-stop'>Unselect this stop</a></div></div>";				
			}			
		}
	}
