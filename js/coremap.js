var coremap = {
	...
	init: function(opts) {
		...
		function preprocess(stopdetails) {
			// TODO: handle this server-side and remove this function.
			for (var i = stopdetails.length - 1; i >= 0; i--) {
				var stop = stopdetails[i];
				stop.id = "MARTA_" + stop.id;
				stop.lon = stop.lng;
				
				if (stop.type === null) stopdetails.splice(i, 1);
			}
		}
